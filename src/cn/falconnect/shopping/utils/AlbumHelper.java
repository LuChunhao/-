...
//		Log.d(TAG, "use time: " + (endTime - startTime) + " ms");
	...
	public void updateIsSelect(String bucketId,String imageId){
		for (int i = 0; i <bucketList.get(bucketId).imageList.size(); i++) {
			if(bucketList.get(bucketId).imageList.get(i).imageId.equals(imageId)){
				bucketList.get(bucketId).imageList.get(i).isSelected = !bucketList.get(bucketId).imageList.get(i).isSelected;
			}
		}
	}
	...
	public void clear(){
		...
		Collection<ImageBucket> c = bucketList.values();
		Iterator<ImageBucket> it = c.iterator();
		for (; it.hasNext();) {
			list.add(it.next());
		}
		for (ImageBucket imageBucket : list) {
			ArrayList<ImageItem> items = (ArrayList<ImageItem>) imageBucket.imageList;
