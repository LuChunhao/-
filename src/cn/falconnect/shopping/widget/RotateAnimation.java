...
public class RotateAnimation extends Animation {  
    ...
    public void setInter(float interpolatedTime){
    ...
    public void applyTransformation(float interpolatedTime, Transformation transformation) {  
    	...
    	super.applyTransformation(inter, transformation);
        if (listener != null) {  
            listener.interpolatedTime(inter);  
            System.out.println("进度=="+inter);
        }  
        ...
        float degree = from + (to - from) * inter;  
        boolean overHalf = (inter > 0.5f);  
        if (overHalf) {  
            // 翻转过半的情况下，为保证数字仍为可读的文字而非镜面效果的文字，需翻转180度。  
            degree = degree - 180;  
        }  
        // float depth = 0.0f;  
        float depth = (0.5f - Math.abs(inter - 0.5f)) * DEPTH_Z;  
