...
public class Banner extends LinearLayout {
	...
	public void changeIndicatorStyle(Rule rule, int height, int color) {
		int verb = RelativeLayout.ALIGN_PARENT_RIGHT;
		if (Rule.LEFT == rule) {
			verb = RelativeLayout.ALIGN_PARENT_LEFT;
		} else if (Rule.RIGHT == rule) {
			verb = RelativeLayout.ALIGN_PARENT_RIGHT;
		} else if (Rule.CENTER == rule) {
			verb = RelativeLayout.CENTER_IN_PARENT;
		}
		int widthDp = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 100, mIndicatorTitle.getContext().getResources()
				.getDisplayMetrics());
